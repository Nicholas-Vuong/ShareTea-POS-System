                ...
                <TabsContent value="productusage" className="space-y-4">
                    <Card className="p-6">
                        <div className="mb-6">
                            ...
                            <p className="text-sm text-muted-foreground mt-1">
                                Detailed breakdown of menu item performance by category
                            </p>
                        </div>
                        ...
                        ) : !analytics || analytics.productUsage.length === 0 ? (
                            <div className="text-center py-12">
                                <Package className="h-16 w-16 mx-auto text-muted-foreground mb-4" />
                                <p className="text-xl font-semibold">No product usage data available</p>
                            </div>
                        ) : (
                            <>
                                {/* Category Summary */}
                                <div className="mb-6">
                                    ...
                                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                                        {Array.from(new Set(analytics.productUsage.map(p => p.category)))
                                            .map(category => {
                                                const categoryItems = analytics.productUsage.filter(p => p.category === category);
                                                const totalRevenue = categoryItems.reduce((sum, item) => sum + item.revenue, 0);
                                                const totalQty = categoryItems.reduce((sum, item) => sum + item.quantitySold, 0);
                                                return { category, totalRevenue, totalQty };
                                            })
                                            .sort((a, b) => b.totalRevenue - a.totalRevenue)
                                            .map((cat, idx) => (
                                                <Card key={cat.category} className="p-4">
                                                    <p className="text-sm text-muted-foreground">{cat.category}</p>
                                                    <p className="text-xl font-bold mt-1">{formatCurrency(cat.totalRevenue)}</p>
                                                    <p className="text-xs text-muted-foreground mt-1">{cat.totalQty} sold</p>
                                                </Card>
                                            ))
                                        }
                                    </div>
                                ...
                                <div className="h-[500px]">
                                    ...
                                    <ResponsiveContainer width="100%" height="100%">
                                        <BarChart
                                            data={analytics.productUsage.slice(0, 15).map(p => ({
                                                name: p.menuItemName.length > 20 ? p.menuItemName.substring(0, 20) + '...' : p.menuItemName,
                                        ...
                                        >
                                            ...
                                            <Tooltip content={<CustomTooltip />} />
                                            <Legend />
                                            <Bar yAxisId="left" dataKey="Quantity Sold" fill={CHART_COLORS.info} radius={[8, 8, 0, 0]} />
                                            <Bar yAxisId="right" dataKey="Revenue" fill={CHART_COLORS.success} radius={[8, 8, 0, 0]} />
                                        </BarChart>
                                    </ResponsiveContainer>
                                </div>
                                ...
                                <div className="h-96 mt-8">
                                    ...
                                    <ResponsiveContainer width="100%" height="100%">
                                        <PieChart>
                                            <Pie
                                                data={Array.from(new Set(analytics.productUsage.map(p => p.category)))
                                                    .map(category => ({
                                                        name: category,
                                                        value: analytics.productUsage
                                                ...
                                                dataKey="value"
                                            >
                                                {Array.from(new Set(analytics.productUsage.map(p => p.category))).map((_, index) => (
                                                    <Cell key={`cell-${index}`} fill={Object.values(CHART_COLORS)[index % Object.values(CHART_COLORS).length]} />
                                                ))}
                                            </Pie>
                                            <Tooltip content={<CustomTooltip />} />
