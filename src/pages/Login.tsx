...
export default function Login() {
        ...
        const translateAllText = async () => {
            ...
            try {
                ...
                const staticTexts = {
                    ...
                    'Sign in without an account': 'Sign in without an account',
                    'Continue as Guest': 'Continue as Guest',
                    'Access the kiosk without creating an account. You can browse and place orders, but you won\'t have access to order history or other account features.': 'Access the kiosk without creating an account. You can browse and place orders, but you won\'t have access to order history or other account features.',
                };
    ...
    const handleSignUp = async () => {
        ...
        if (!email.trim()) {
            const title = language !== 'en' ? await translateText('Email required', language, 'en') : 'Email required';
            const desc = language !== 'en' ? await translateText('Please enter your email address', language, 'en') : 'Please enter your email address';
            toast({
                title,
                description: desc,
                variant: 'destructive',
        ...
        setLoading(true);
        try {
            const user = await api.signUp(username, password, fullName, selectedRole, trimmedEmail);
            setUser({
                userId: user.userId,
            ...
            <div className="w-full max-w-6xl grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <Tabs value={isSignUp ? 'signup' : 'login'} onValueChange={(v) => setIsSignUp(v === 'signup')}>
                        ...
                        <TabsContent value="login" className="space-y-4">
                            <Button
                                ...
                                onClick={async () => {
                                    try {
                                        ...
                                        const title = language !== 'en' ? await translateText('Google login failed', language, 'en') : 'Google login failed';
                                        const desc = error.message || 'Failed to initiate Google login. Please try again.';
                                        toast({
                                            title,
                                            description: desc,
                                            variant: 'destructive',
                            ...
                            >
                                ...
                                {loading ? t('Signing in...') : 'Continue with Google'}
                            </Button>
                        ...
                        <TabsContent value="signup" className="space-y-4">
                            ...
                            <div className="space-y-2">
                                ...
                                <p className="text-xs text-muted-foreground">
                                    {t('Email is required for account creation')}
                                </p>
                            </div>
                ...
                <Card className="p-8 space-y-6 flex flex-col justify-center">
                    <div className="text-center space-y-4">
                        <h2 className="text-3xl font-bold text-primary">{t('Sign in without an account')}</h2>
                        <p className="text-muted-foreground">
                            {t('Access the kiosk without creating an account. You can browse and place orders, but you won\'t have access to order history or other account features.')}
                        </p>
                    </div>
                    <Button
